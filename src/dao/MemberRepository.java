...
public class MemberRepository extends Repository {
	...
	public Member logon(String username) {
		Member member = jdbc.queryForObject("SELECT MemberId, Username, Password FROM Member WHERE Username = ?",
				new MemberMapper(), username);
	...
	public List<Role> getRolesByMemberId(long id) {
		return jdbc.query(
				"SELECT Role.RoleId, RoleName FROM Role INNER JOIN MemberInRole ON Role.RoleId = MemberInRole.RoleId WHERE MemberId = ?",new RowMapper<Role>() {
